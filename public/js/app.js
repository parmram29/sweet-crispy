// ============================================================
// Application bootstrap. Wires the shared services (ApiClient,
// MenuStore, Cart, Toast) to the Router and each page class, then
// kicks off the initial render. This is the only file that knows
// about all the pieces — everything else only depends on what's
// passed into its constructor, so any page/service can be tested
...
class App {
  constructor() {
    this.menuStore = new MenuStore(api);
    this.cart = new Cart();
    ...
    this.router.onEnter('admin', () => this.adminPage.onEnter());

    this.bindNav();
  }

  // Single delegated listener for the top nav bar (logo, hamburger, links, staff).
  bindNav() {
  ...
  async start() {
    ...
    await this.orderPage.handlePaymentReturn(this.router);
  }
}
